...
export function TypingAnimation({
  ...
  const { displayedText, currentWordIndex, currentCharIndex, phase } = animState
  ...
  const [prevAnimationSourceKey, setPrevAnimationSourceKey] =
    useState(animationSourceKey)

  if (prevAnimationSourceKey !== animationSourceKey) {
    ...
    if (shouldStart && wordsToAnimate.length > 0) {
      ...
      timeout = setTimeout(() => {
        ...
        switch (phase) {
          case "typing":
            if (currentCharIndex < graphemes.length) {
              ...
              if (hasMultipleWords || loop) {
                ...
                if (!isLastWord || loop) {
                  setAnimState((prev) => ({ ...prev, phase: "pause" }))
                }
              }
            }
            break
          ...
          case "pause":
            setAnimState((prev) => ({ ...prev, phase: "deleting" }))
            break
