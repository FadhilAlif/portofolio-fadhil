...
export function FadhilSignatureEffect({ className, speed = 1, onAnimationComplete, ...props }: Props) {
  const calc = (x: number) => x * speed
  ...
  return (
    ...
    >
      ...
      <defs>
        <clipPath id="fadhil-reveal">
          <motion.rect
            ...
            initial={{ width: 0 }}
            animate={{ width: 400 }}
            transition={{ duration: calc(2), ease: "easeInOut" }}
            onAnimationComplete={onAnimationComplete}
          />
        </clipPath>
      </defs>

      {/* Background text: very faint or invisible, used for the outline effect if desired, here we just use it as the base drawing */}
      <text
        x="50%"
        y="50%"
        textAnchor="middle"
        dominantBaseline="middle"
        style={{ fontFamily: "'Caveat', 'Segoe Script', 'Brush Script MT', cursive", fontSize: "110px", fontWeight: "600" }}
        fill="transparent"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeOpacity="0.1"
      ...
      {/* Foreground text: completely invisible initially, revealed by the clip path animating its width */}
      <text
        x="50%"
        y="50%"
        textAnchor="middle"
